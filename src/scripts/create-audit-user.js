...
async function main() {
  const orgCode = process.env.SEED_AUDIT_ORG_CODE || 'ORG001';
  const orgName = process.env.SEED_AUDIT_ORG_NAME || 'Zone Idea Demo Organization';
  const username = process.env.SEED_AUDIT_USERNAME || 'audit';
  const password = process.env.SEED_AUDIT_PASSWORD || 'Admin@123456';
  const name = process.env.SEED_AUDIT_NAME || 'Audit Officer';
  const email = process.env.SEED_AUDIT_EMAIL || 'audit@example.com';
  const phone = process.env.SEED_AUDIT_PHONE || '0800000004';
  ...
  try {
    ...
    const [orgRows] = await connection.execute(
      `SELECT id FROM organizations WHERE code = :code LIMIT 1`,
      { code: orgCode },
    );
    ...
    await connection.execute(
      `INSERT INTO admin_users (
        ...
        :organizationId, :usernameHash, :passwordHash, 'audit',
    ...
    logger.info({ organizationId, orgCode, username }, 'Audit seed completed');
    console.log(`audit organization code: ${orgCode}`);
    console.log(`audit username: ${username}`);
    console.log(`audit password: ${password}`);
  } catch (error) {
...
main().catch((error) => {
  logger.error({ error }, 'Audit seed failed');
  process.exit(1);
});
